...
- 完善 AI 实机测试授权流程：先按当前会话实际暴露的工具判断能否控制本机 Steam、启动器和
  游戏，不再按 Codex、Cursor、Claude 或 Gemini 等客户端名称推断能力；Codex 可读取时优先
  使用只读 `planType`、额度及计费元数据，其他运行时使用其可信元数据。API 计费、
  Free／Go／Plus 及未知额度继续确认费用或额度接受度，高额度方案在用户已明确要求自主测试时
  不重复询问。
  用户因额度或费用放弃后，必须提供覆盖入口、分支、存档、奖励、状态及日志截图的详细
  人工测试方案；缺少套餐元数据本身不得被当作 API 计费证据。
